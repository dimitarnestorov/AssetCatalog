#import <AssetCatalog/AssetCatalog.h>

#import "Private Headers/CoreUI.h"
#import "Private Headers/CoreUI+TV.h"
...
- (void)readWithCompletionHandler:(void (^__nonnull)(void))callback progressHandler:(void (^__nullable)(double progress))progressCallback
{
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if ([AssetCatalog isProThemeStoreAtPath:catalogPath]) {
            ...
            for (CUINamedImage *namedImage in [self imagesNamed:imageName]) {
                ...
                @autoreleasepool {
                    ...
                    if ([namedImage isKindOfClass:[CUINamedLayerStack class]]) {
                        ...
                        filename = [AssetCatalog filenameForAssetNamed:namedImage.name scale:namedImage.scale presentationState:kCoreThemeStateNone];
...
- (void)readThemeStoreWithCompletionHandler:(void (^__nonnull)(void))callback progressHandler:(void (^__nullable)(double progress))progressCallback
{
    ...
    [[self.catalog _themeStore].themeStore.allAssetKeys enumerateObjectsWithOptions:0 usingBlock:^(CUIRenditionKey * _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        @try {
            ...
            NSString *filename = [AssetCatalog filenameForAssetNamed:[AssetCatalog cleanupRenditionName:rendition.name] scale:rendition.scale presentationState:key.themeState];
...
- (NSMutableArray<NSDictionary<NSString *, NSObject *> *> *)readImages:(NSString *)name distinguishCatalogsFromThemeStores:(BOOL)distinguishCatalogsFromThemeStores
{
    NSMutableArray<NSDictionary<NSString *, NSObject *> *> *mutableImages = [NSMutableArray new];
    NSString *catalogPath = self.fileURL.path;
    ...
    // bundle is nil for some reason
    if (!catalogPath) {
        @throw [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"Unable to find asset catalog path"}];
    }
    
    NSError *catalogError;
    CUICatalog *catalog = [[CUICatalog alloc] initWithURL:[NSURL fileURLWithPath:catalogPath] error:&catalogError];
    if (catalogError) {
        @throw catalogError;
    }
    
    if ([AssetCatalog isProThemeStoreAtPath:catalogPath]) {
        @throw [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Pro asset catalogs are not supported"}];
    }
    
    if (distinguishCatalogsFromThemeStores) {
        if (!catalog.allImageNames.count || ![catalog respondsToSelector:@selector(imageWithName:scaleFactor:)]) {
            // CAR is a theme file not an asset catalog
            // return [self readThemeStoreWithCompletionHandler:callback progressHandler:progressCallback];
            @throw [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:130 userInfo:@{NSLocalizedDescriptionKey: @"Unexpected code path"}];
        }
    } else {
        // return [self readThemeStoreWithCompletionHandler:callback progressHandler:progressCallback];
        @throw [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:130 userInfo:@{NSLocalizedDescriptionKey: @"Unexpected code path"}];
    ...
    for (CUINamedImage *namedImage in [AssetCatalog imagesNamed:name inCatalog:catalog]) {
        @autoreleasepool {
            if (namedImage == nil) {
                continue;
            }
            ...
            NSDictionary *desc = [AssetCatalog imageDescriptionWithName:namedImage.name filename:filename representation:imageRep];
            if (!desc) {
                return;
            }
    ...
    if (!mutableImages.count) {
        NSLog(@"AssetCatalog warning: Failed to load images");
    }
#endif
...
+ (NSArray <CUINamedImage *> *)imagesNamed:(NSString *)name inCatalog:(CUICatalog *)catalog
{
    NSMutableArray <CUINamedImage *> *images = [[NSMutableArray alloc] initWithCapacity:3];
...
- (NSDictionary *)imageDescriptionWithName:(NSString *)name filename:(NSString *)filename representation:(NSBitmapImageRep *)imageRep
{
    if (_resourceConstrained) {
        ...
        NSImage *thumbnail = [AssetCatalog constrainImage:originalImage toSize:self.thumbnailSize];
